...
async function loadOrders() {
    try {
        const res = await window.wrappedFetch(`${API_BASE}/orders`);
...
async function loadCustomers() {
    try {
        const res = await window.wrappedFetch(`${API_BASE}/customers`);
        const result = await res.json();
        if (result.success) {
...
async function saveCustomer(e) {
    ...
    try {
        let res, result;
        if (customerId) {
            res = await window.wrappedFetch(`${API_BASE}/customers/${customerId}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(data)
            });
        } else {
            res = await window.wrappedFetch(`${API_BASE}/customers`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(data)
            });
