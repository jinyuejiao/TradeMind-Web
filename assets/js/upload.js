...
async function uploadImage() {
    ...
    try {
        const response = await window.wrappedFetch(`${API_BASE}/api/v1/ai/upload`, {
            method: 'POST',
            body: formData
        });
...
async function confirmAndSave() {
    ...
    try {
        ...
        if (customerInfo && customerInfo.nickname) {
            const customerRes = await window.wrappedFetch(`${API_BASE}/api/v1/crm/customers`, {
                method: 'POST',
        ...
        const orderRes = await window.wrappedFetch(`${API_BASE}/api/v1/rd/orders`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                ...data,
        ...
        if (orderResult.order) {
            ...
            if (productInfo && productInfo.stock_impact === '需扣减库存') {
                await window.wrappedFetch(`${API_BASE}/api/v1/rd/inventory/deduct`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
            ...
            if (productInfo && productInfo.supplier_action === '需向对应供应商下单') {
                await window.wrappedFetch(`${API_BASE}/api/v1/supp/supplier-orders`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        product_name: productInfo.product_name,
