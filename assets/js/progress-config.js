...
async function loadProgressConfig() {
    try {
        const res = await window.wrappedFetch(`${API_BASE}/progress-config`);
...
async function saveStep(e) {
    ...
    try {
        let res, result;
        if (stepId) {
            res = await window.wrappedFetch(`${API_BASE}/progress-config/${stepId}`, {
                method: 'PUT',
            ...
            res = await window.wrappedFetch(`${API_BASE}/progress-config`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(data)
            });
...
async function deleteStep(stepId) {
    ...
    try {
        const res = await window.wrappedFetch(`${API_BASE}/progress-config/${stepId}`, {
            method: 'DELETE'
        });
...
async function saveProgressConfig() {
    try {
        await window.wrappedFetch(`${API_BASE}/progress-config/reorder`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
