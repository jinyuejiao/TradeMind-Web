/**
 * TradeMind — OSS 上传：工作台识图/订单图片与语音走 AIService 服务端；问题反馈走 TenantService。
 * initClient/uploadBlob 保留供兼容，识图请使用 uploadDashboardImage。
 */
(function () {
    'use strict';
    ...
    function uploadBlob(objectKey, blob) {
        if (objectKey && String(objectKey).indexOf('/image/') >= 0) {
            return uploadDashboardImage(blob);
        }
    ...
    function uploadDashboardImage(blob) {
        ...
        return window.wrappedFetch('/api/v1/ai/image/upload', {
            method: 'POST',
            body: formData
        }).then(function (res) {
