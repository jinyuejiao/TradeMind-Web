...
window.ProductModule = {
    ...
    mapProductFromApi: function(apiProduct) {
        ...
        return {
            ...
            purchasePrice: (function () {
                var raw = apiProduct.costPrice != null ? apiProduct.costPrice
                    : (apiProduct.purchasePrice != null ? apiProduct.purchasePrice : null);
            ...
            stock: stockNum,
            totalStock: apiProduct.totalStock != null ? Number(apiProduct.totalStock) : null,
            warehouseStock: apiProduct.warehouseStock != null ? Number(apiProduct.warehouseStock) : null,
            salesVolume: apiProduct.salesCount || apiProduct.salesVolume,
            icon: apiProduct.productIcon || apiProduct.icon || 'package',
            skuId: apiProduct.skuId != null ? Number(apiProduct.skuId)
                : (apiProduct.sku_id != null ? Number(apiProduct.sku_id) : null),
            spuId: apiProduct.spuId != null ? Number(apiProduct.spuId)
                : (apiProduct.spu_id != null ? Number(apiProduct.spu_id) : null),
    ...
    loadProducts: async function() {
        ...
        try {
            ...
            if (this.filterState && this.filterState.warehouseId != null) {
                qs = '?warehouseId=' + encodeURIComponent(this.filterState.warehouseId);
            }
            console.log('[ProductModule] 准备调用API: /api/v1/rd/products' + qs);
            const response = await window.wrappedFetch('/api/v1/rd/products' + qs, {
                method: 'GET'
            });
    ...
    // 筛选状态（与 products.category_id / supplier_id / warehouse 对齐）
    filterState: {
        categoryId: null,
        supplierId: null,
        warehouseId: null,
    ...
    bindFilterDropdownDelegates: function() {
        ...
        var self = this;
        if (catDd.dataset.tmFilterDelegated !== '1') {
        ...
        if (supDd.dataset.tmFilterDelegated !== '1') {
        ...
        if (stkDd.dataset.tmFilterDelegated !== '1') {
        ...
        if (whDd && whDd.dataset.tmFilterDelegated !== '1') {
            whDd.dataset.tmFilterDelegated = '1';
    ...
    selectWarehouseFilter: async function(warehouseId, displayName) {
        ...
        if (this.filterState.warehouseId != null) {
            if (label) label.textContent = displayName || '仓库';
        ...
        const whDd = document.getElementById('warehouse-dropdown');
        if (whDd) whDd.classList.add('hidden');
        const filterEl = document.getElementById('warehouse-filter');
        if (filterEl) {
            const caretIcon = filterEl.querySelector('.filter-caret-icon');
            if (caretIcon) {
                caretIcon.classList.remove('ph-caret-up', 'rotate-180', 'text-teal-500');
        ...
        await this.loadProducts();
        if (this.spuListMode) {
            await this.loadAndRenderSpuList();
        } else {
            this.filterProducts();
        }
    },
    ...
    resetFilters: function() {
        ...
        var prevWh = this.filterState.warehouseId;
        this.filterState = {
            categoryId: null,
            supplierId: null,
            warehouseId: null,
        ...
        if (prevWh != null) {
            ...
            this.loadProducts().then(function () {
                if (self.spuListMode) self.loadAndRenderSpuList();
                else self.filterProducts();
            });
        } else {
            this.filterProducts();
            if (this.spuListMode) this.loadAndRenderSpuList();
        }
    },
    ...
    syncSpuViewButton: function () {
        ...
        // 文案固定为「SPU 视图」，仅用选中态表达开关
        var texts = [];
        btn.childNodes.forEach(function (n) {
            if (n.nodeType === 3 && String(n.textContent || '').trim()) texts.push(n);
        });
        if (texts.length) {
            texts[0].textContent = ' SPU 视图';
        }
        btn.title = on ? '当前为 SPU 树形列表，点击切回扁平列表' : '切换 SPU 树形列表';
        btn.setAttribute('aria-pressed', on ? 'true' : 'false');
    },

    toggleSpuListView: async function () {
        this.spuListMode = !this.spuListMode;
        this.syncSpuViewButton();
        if (this.spuListMode) {
            await this.loadAndRenderSpuList();
        } else {
            this.renderProducts(this.filteredProducts && this.filteredProducts.length
                ? this.filteredProducts
                : (this.products || []), { resetPage: false });
        }
    },

    loadAndRenderSpuList: async function () {
        try {
            var categoryId = this.filterState && this.filterState.categoryId != null
                ? Number(this.filterState.categoryId) : null;
            if (categoryId != null && isNaN(categoryId)) categoryId = null;
            var warehouseId = this.filterState && this.filterState.warehouseId != null
                ? Number(this.filterState.warehouseId) : null;
            if (warehouseId != null && isNaN(warehouseId)) warehouseId = null;
            var qs = ['pageNo=1', 'pageSize=200'];
            ...
            var resp = await window.wrappedFetch('/api/v1/rd/products/spu?' + qs.join('&'), { method: 'GET' });
            var data = await window.handleApiResponse(resp);
            var payload = data && data.data ? data.data : data;
            var records = payload && Array.isArray(payload.records) ? payload.records
                : (Array.isArray(payload) ? payload : []);
            var q = (this.filterState && this.filterState.searchText)
                ? String(this.filterState.searchText).trim().toLowerCase()
                : '';
            if (q) {
                records = records.filter(function (r) {
                    var name = String(r.name || r.spu_name || '').toLowerCase();
            ...
            console.error('[ProductModule] 加载 SPU 列表失败', e);
            if (window.TM_UI && window.TM_UI.showNotification) {
                window.TM_UI.showNotification('加载 SPU 列表失败: ' + (e.message || e), 'error');
            }
            this.spuListMode = false;
            this.syncSpuViewButton();
        }
    },

    renderSpuList: function (records) {
        records = records || [];
        this.productTotal = records.length;
        this.productTotalPages = 1;
        this.productCurrentPage = 1;
        this.renderSpuDesktopTable(records);
        this.renderSpuMobileCards(records);
    ...
    renderSpuDesktopTable: function (records) {
        var tbody = document.querySelector('#existingProdTable tbody');
        if (!tbody) return;
        var self = this;
        if (!records.length) {
            tbody.innerHTML = '<tr class="hidden md:table-row"><td colspan="5" class="px-6 py-12 text-center">'
                + '<div class="flex flex-col items-center gap-3">'
                + '<i class="ph ph-tree-structure text-4xl text-slate-300"></i>'
                + '<p class="text-slate-400 font-bold">暂无 SPU 商品</p></div></td></tr>';
            return;
        }
        var html = '';
        records.forEach(function (spu) {
            var spuId = spu.spu_id != null ? spu.spu_id : spu.spuId;
            ...
            if (expanded) {
                ...
                if (!skus.length) {
                    ...
                    skus.forEach(function (sku) {
                        ...
                        var skuStockLabel = self.formatStockQtyDisplay(stock, skuMeta);
                        var legacyId = sku.legacyProductId != null ? sku.legacyProductId : sku.legacy_product_id;
                        var openFn = legacyId
    ...
    renderSpuMobileCards: function (records) {
        var container = document.getElementById('mobile-product-cards');
        if (!container) return;
        var self = this;
        if (!records.length) {
            container.innerHTML = '<div class="py-10 px-4 text-center">'
                + '<i class="ph ph-tree-structure text-3xl text-slate-300"></i>'
                + '<p class="text-slate-400 text-sm font-bold mt-2">暂无 SPU 商品</p></div>';
            return;
        }
        container.innerHTML = records.map(function (spu) {
            var spuId = spu.spu_id != null ? spu.spu_id : spu.spuId;
            var name = self.escHtml(spu.name || ('SPU#' + spuId));
            var skuCount = Number(spu.sku_count != null ? spu.sku_count : (spu.skuCount || 0)) || 0;
            var totalStock = Number(spu.total_stock != null ? spu.total_stock : (spu.totalStock || 0)) || 0;
            ...
            if (expanded) {
                if (!skus.length) {
                    childHtml = '<p class="text-[11px] text-slate-400 px-1 py-2">加载规格中…</p>';
                } else {
            ...
            return '<div class="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm">'
                ...
                + (expanded ? ('<div class="mt-3 space-y-2">' + childHtml + '</div>') : '')
                + '</div>';
        }).join('');
    },

    toggleSpuExpand: async function (spuId) {
        if (!spuId) return;
        var key = String(spuId);
        if (this.spuExpandedMap[key] || this.spuExpandedMap[spuId]) {
            delete this.spuExpandedMap[key];
            delete this.spuExpandedMap[spuId];
    ...
    openSpuDetail: async function (spuId) {
        try {
            ...
            var skus = detail && detail.skus ? detail.skus : [];
            var hit = skus.find(function (s) { return s.isDefault || s.is_default; }) || skus[0];
            var legacyId = hit && (hit.legacyProductId != null ? hit.legacyProductId : hit.legacy_product_id);
            if (legacyId) {
                this.openProductDetail(legacyId);
                return;
            }
            if (window.TM_UI && window.TM_UI.showNotification) {
                window.TM_UI.showNotification('该商品尚未关联可编辑档案', 'warning');
            }
        } catch (e) {
            console.error('[ProductModule] openSpuDetail failed', e);
        }
    },

    renderProducts: function(productList, options) {
        if (this.spuListMode) {
            this.loadAndRenderSpuList();
            return;
        }
        const opts = options || {};
    ...
    initWarehouseOptions: function() {
        console.log('[ProductModule] initWarehouseOptions 被调用 ===');
        ...
        (this.warehouses || []).forEach(function (warehouse) {
            const id = warehouse.id != null ? warehouse.id : warehouse.warehouseId;
            const nm = warehouse.name || warehouse.warehouseName || '未命名仓库';
            if (id == null || id === '') return;
            parts.push(
                '<button type="button" data-warehouse-key="' + String(Number(id)) + '" class="w-full text-left px-4 py-3 text-sm font-bold text-slate-800 hover:bg-teal-50 hover:text-teal-700 transition-all border-b border-slate-50 last:border-b-0">' + escHtml(nm) + '</button>'
            );
        });
        container.innerHTML = parts.join('');
    },
    ...
    /** 入库/调拨后清 SPU 展开缓存，避免父级已更新、子行仍旧 */
    invalidateInventoryCaches: function () {
        this.spuSkuCache = {};
        if (window.TM_MasterDataCache) {
            if (typeof window.TM_MasterDataCache.invalidateAll === 'function') {
                try { window.TM_MasterDataCache.invalidateAll(); } catch (e) { /* ignore */ }
            } else if (typeof window.TM_MasterDataCache.invalidateSpu === 'function') {
                Object.keys(this.spuExpandedMap || {}).forEach(function (id) {
                    try { window.TM_MasterDataCache.invalidateSpu(id); } catch (e2) { /* ignore */ }
                });
            }
        }
    },

    resolveUnitMetaForSpu: function (spu) {
        var name = (spu && (spu.name || spu.spu_name || '')).trim();
        var hit = null;
        if (name && Array.isArray(this.products)) {
            hit = this.products.find(function (p) { return String(p.name || '').trim() === name; });
        }
        if (!hit && spu) {
            var lid = spu.legacy_product_id || spu.legacyProductId;
            if (lid != null && Array.isArray(this.products)) {
                hit = this.products.find(function (p) { return String(p.id) === String(lid); });
            }
        }
        return {
            baseUnit: (hit && hit.baseUnit) || '件',
            unitConversions: (hit && hit.unitConversions) || []
        };
    },

    formatStockQtyDisplay: function (qty, unitMeta) {
        unitMeta = unitMeta || {};
        return this.formatCompoundStockDisplay({
            stock: qty,
            baseUnit: unitMeta.baseUnit || '件',
            unitConversions: unitMeta.unitConversions || []
        });
    },
    ...
    closeClearanceModal: function() {
        // 委托主壳 TM_BizAI，避免覆盖真实促销弹窗关闭逻辑
        if (window.TM_BizAI && typeof window.TM_BizAI.closeClearanceModal === 'function') {
            window.TM_BizAI.closeClearanceModal();
            return;
        }
        if (typeof window.closeClearanceModal === 'function'
            && window.closeClearanceModal !== window.ProductModule.closeClearanceModal) {
            window.closeClearanceModal();
        }
    },
...
window.initWarehouseOptions = function() { window.ProductModule.initWarehouseOptions(); };
window.selectWarehouseFilter = function(warehouseId, displayName) { window.ProductModule.selectWarehouseFilter(warehouseId, displayName); };
...
// 不覆盖主壳 closeClearanceModal；ProductModule.closeClearanceModal 仅作兼容转发
if (typeof window.closeClearanceModal !== 'function') {
    window.closeClearanceModal = function () {
        if (window.TM_BizAI && typeof window.TM_BizAI.closeClearanceModal === 'function') {
            window.TM_BizAI.closeClearanceModal();
        } else if (window.ProductModule && typeof window.ProductModule.closeClearanceModal === 'function') {
            window.ProductModule.closeClearanceModal();
        }
    };
}
